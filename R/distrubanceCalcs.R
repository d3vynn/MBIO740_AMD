...
for (l in 1:length(ls)){
  ...
  ff <- which(round(FullModel$FishPopulation[49:100,1]) >= 
          round(FullModel$FishPopulation[50,1]))
  dd <- ff[2:length(ff)] - ff[1:(length(ff)-1)]
  FishRecv <- dd[which(dd > 1)]
  
  ff <- which(round(FullModel$Harvest[49:100,1]) >= 
                round(FullModel$Harvest[50,1]))
  dd <- ff[2:length(ff)] - ff[1:(length(ff)-1)]
  HarvRecv <- dd[which(dd > 1)]
  
  ff <- which(round(FullModel$Lambdas[49:100],2) >= 
                round(FullModel$Lambdas[50],2))
  dd <- ff[2:length(ff)] - ff[1:(length(ff)-1)]
...
save(dataf, file=paste0(BaseD,"Disturbance_full.rdata"))
save(recovf, file=paste0(BaseD,"Disturbance_times.rdata"))
